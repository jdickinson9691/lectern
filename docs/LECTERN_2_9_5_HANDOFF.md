...
- The Campaign Dashboard reports party DPR/HPR, critical-hit/miss leaders, and party leaders for all 13 standard 5E damage types from normalized combat-log data, including tied leaders and attribution coverage. Damage-type leaders use component-level applied values, with a conservative single-type fallback for older rows. Fantasy Grounds imports/reprocessing and local combat actions populate the normalized statistics fields.
