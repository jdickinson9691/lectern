...
- Fantasy Grounds Sync 1.3.1 carries prepared-participant combat statistics and live combat events into Lectern, reads 2024 character armor class, skips unnamed placeholder characters, and excludes loaded-module reference battles from campaign encounters. Explicit `/lectern-start` and `/lectern-end` commands create durable named sessions that survive extension reloads, retain accumulated events, and preserve the final non-empty combatant roster after tracker clearing. `/lectern-reset confirm` and Lectern's previewed, automatically backed-up clear action provide a clean testing reset while preserving local data. Attack events use the 5E ruleset's authoritative post-resolution result and effect-adjusted defense; applied damage reports reductions or increases and refuses stale/manual actor attribution.
