...
Lectern Sync 1.4.3 imports equipped weapons and armor from 5E character inventories and records the damage types resolved by the ruleset. Mixed damage remains component-aware, every target of a multi-target action retains its roll attribution, and expanded Combat Dashboard rows preserve rolled and applied values after resistance, immunity, vulnerability, wards, and similar adjustments. Applied component totals never exceed the target's actual HP loss. Manual wound edits are labeled with an unknown damage type.
