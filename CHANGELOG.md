...
- Added Lectern Sync 1.4.8 explicit overkill and damage-mitigation evidence. Fantasy Grounds overflow is now recorded separately from resistance or damage reduction, vulnerability increases, temporary-HP absorption, and actual HP damage. Resolved damage also retains eligible effect contributors such as Sneak Attack, Hunter's Mark, and Divine Smite with their effect node, label, component, dice, modifier, and damage types. Healing preserves the healer and originating spell/ability for dice-based and fixed healing such as Lay on Hands; manual wound edits remain unattributed. Each resolved save target preserves the originating caster and spell/ability, save ability, actual DC, final save total, and success/failure without deriving the outcome from armor class.
- Added authoritative Combat Tracker effect lifecycle events. Added and removed effects now preserve the effect label, target, duration, source reference/name, and explicit source-attribution method without guessing why an effect ended.
- Added a validated, versioned offline Combat Narrative library with deterministic phrase selection, a heroic military chronicle profile, more than seventy grounded variants, strict placeholder and forbidden-language checks, and documented event ownership and sample coverage.
